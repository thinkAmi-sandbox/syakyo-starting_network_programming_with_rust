...
// リスニングソケットのトークン
// 監視対象となるソケット間の識別は、mioで定義されているTokenを使用
...
// イベントループを基本として、シングルスレッドで動作
// ソケットのI/O多重化とソケットのノンブロッキングを両方利用
struct WebServer {
    listening_socket: TcpListener,
    // サーバに接続されているクライアントを管理するハッシュテーブル
...
impl WebServer {
    ...
    fn new(addr: &str) -> Result<Self, failure::Error> {
        // エラー移譲(末尾"?")による処理(エラーのときは関数の戻り値になる)
        let address = addr.parse()?;
        let listening_socket = TcpListener::bind(&address)?;
        Ok(WebServer {
            listening_socket,
            // HashMap -> キーとバリューを持つマップ
    ...
    fn run(&mut self) -> Result<(), failure::Error> {
        let poll = Poll::new()?;
        // サーバーソケットの状態お監視対象に登録する
        poll.register(
            &self.listening_socket,
            SERVER,
            Ready::readable(),
            // 準備完了の間は常に読み込むを行う (レベルトリガー)
            // <-> 準備完了に切り替わったときのみ読み込みを行う (エッジトリガー)
            PollOpt::level(),
        )?;
        ...
        loop {
            ...
            // イベントは一度に複数回発生する可能性があるため、forで回す
            for event in &events {
                match event.token() {
                    SERVER => {
                        // 「リスニングソケットの読み込み準備完了」イベントが発生
    ...
    -> Result<(), failure::Error> {
        ...
        if event.readiness().is_readable() {
            ...
            if nbytes != 0 {
                // responseの参照されている値として設定する？
                // https://qiita.com/lex_naturalis/items/dbef84b7788efbd30e21
                ...
                *response = make_response(&buffer[..nbytes])?;
                // 書き込み操作の可否を監視対象に入れる
                poll.register(stream, Token(conn_id), Ready::writable(),
                PollOpt::edge())?;
            } else {
                // 通信終了
...
fn make_response(buffer: &[u8]) -> Result<Vec<u8>, failure::Error> {
    ...
    if method == "GET" {
        let file = match File::open(path) {
            Ok(file) => file,
            Err(_) => {
                // パーミッションエラーなども含まれるが、簡略化のためNOT FOUND
...
fn create_msg_from_code(status_code: u16, msg: Option<Vec<u8>>) -> Result<Vec<u8>, failure::Error> {
    match status_code {
        ...
        400 =>
            Ok("HTTP1.0 400 Bad Request\r\n\
                    Server: mio webserver\r\n\r\n"
                .to_string()
                .into_bytes()),
        404 =>
            Ok("HTTP1.0 404 Not Found\r\n\
                    Server: mio webserver\r\n\r\n"
                .to_string()
                .into_bytes()),
        501 =>
            Ok("HTTP1.0 501 Not Implemented\r\n\
                    Server: mio webserver\r\n\r\n"
                .to_string()
                .into_bytes()),
