...
fn main() {
    ...
    // ヒープ上に DhcpServer 構造体を確保し、複数のスレッドから共有するため、Arcを利用している
    // スレッドから共有されるので、スレッドを生成する際はそのクローンを作成してそれを移動する
                ...
                thread::spawn(move || {
                    if let Some(dhcp_packet) = DhcpPacket::new(recv_buf[..size].to_vec()) {
                        if dhcp_packet.get_op() != BOOTREQUEST {
                            // クライアントからのリクエストがなければ無視
                            return;
                        }

                        if let Err(e) = dhcp_handler(
                            &dhcp_packet, &transmission_socket, cloned_dhcp_server)
                        {
                            error!("{}", e);
                        }
                    }
...
fn dhcp_handler(
    ...
    let client_mac_addr = packet.get_chaddr();
    ...
    match message_type {
        ...
        _ => {
            ...
            let msg = format!(
                // TODO フォーマットの意味
                "{:x}: received unimplemented message, message_type: {}", transaction_id, message_type
            );
 ...
 * 利用できるアドレスを選択して、DHCPOFFERメッセージを返却
 */
...
fn select_lease_ip(
    ...
    // Requested Ip Addr オプションがあり、利用可能ならば、そのIPアドレスを返却
    if let Some(ip_to_be_leased) = obtain_available_ip_from_requested_option(
        dhcp_server,
        &received_packet
    )
    {
...
// リスト5.21
/**
 * REQUESTメッセージのオプションに server_identifier が含まれているときのハンドラ
...
fn dhcp_request_message_handler_responded_to_offer(
    ...
    // DHCPOFFERメッセージに対する応答の場合
    // 必ず'requested IP address'に割当予定のIPアドレスが含まれる
    ...
    let count = {
        ...
        let count = database::count_records_by_mac_addr(&tx, client_mac_addr)?;
        match count {
            // レコードがないならinsert
            0 => database::insert_entry(&tx, client_mac_addr, ip_to_be_leased)?,
            // レコードがあるならUpdate
            _ => database::update_entry(&tx, client_mac_addr, ip_to_be_leased, 0)?,
        ...
        util::send_dhcp_broadcast_response(
            soc,
            dhcp_packet.get_buffer()
        )?;
...
// リスト5.22
/**
 * DHCPREQUEST メッセージのオプションに server_identifier が含まれない場合のハンドラ
...
fn dhcp_request_message_handler_to_reallocate(
    ...
    dhcp_server: Arc<DhcpServer>,
    received_packet: &DhcpPacket,
    client_mac_address: MacAddr,
    soc: &UdpSocket,
) -> Result<(), failure::Error> {
    info!("{:x}: received DHCPREQUEST without server_id", xid);

    if let Some(requested_ip) = received_packet.get_option(
        Code::RequestedIpAddress as u8) {
        // クライアントが、以前割り当てられたIPアドレスを記憶していて、
        // 再起動状態にある時
        let requested_ip = util::u8_to_ipv4addr(&requested_ip)
            .ok_or_else(|| failure::err_msg("Failed to convert ip address"))?;

        let con = dhcp_server.db_connection.lock().unwrap();
        match database::select_entry(&con, client_mac_address)? {
            Some(ip) => {
                if ip == requested_ip && dhcp_server.network_addr.contains(ip) {
                    // 以前割り当てられたIPアドレスと要求されたIPアドレスが一致しており、
                    // ネットワークに含まれている場合はACKを返す
                    let dhcp_packet = make_dhcp_packet(
                        &received_packet,
                        &dhcp_server,
                        DHCPACK,
...
fn dhcp_release_message_handler(
    ...
    // 論理削除
    // DHCPOFFERメッセージを返す際に、解放済のIPアドレスを再割り当てする可能性があるため
    database::delete_entry(&tx, client_mac_addr)?;
    tx.commit()?;
