...
pub struct DhcpPacket {
    // オプション領域を持つので、コンパイル時にバッファのサイズを決定できないため、配列ではなくVec
    buffer: Vec<u8>,
}
...
impl DhcpPacket {
    ...
    pub fn set_chaddr(&mut self, chaddr: MacAddr) {
        ...
        // ここだけ CHADDR..SNAMEでないのは、chaddrフィールが16オクテット確保されているため
        // 今回はMACアドレスしかこのフィールドに入らないので、MACアドレスのサイズである6オクテットを確保している
...
pub struct DhcpServer {
    // 以下の2つは複数スレッドから同時にmutアクセスされるため、ロックを用いている
    address_pool: RwLock<Vec<Ipv4Addr>>,  // 利用可能なアドレス
    pub db_connection: Mutex<Connection>,  // ConnectionはSyncを実装しないので、RwLockではダメ
...
impl DhcpServer {
    ...
    fn init_address_pool(
        ...
        // ネットワークのすべてのIPアドレスから、使用されているIPアドレスを覗いたものをアドレスプールとする
        let mut addr_pool: Vec<Ipv4Addr> = network_addr_with_prefix
            .iter()
            .filter(|addr| !used_ip_addrs.contains(addr))
            .collect();

        // 気持ち的にIPアドレスの若い方から割当てたいので、逆順にする
        // 取り出すときは、末尾から pop() を行う
        addr_pool.reverse();
