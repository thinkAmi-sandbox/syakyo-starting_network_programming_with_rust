...
pub fn is_ipaddr_available(target_ip: Ipv4Addr) -> Result<(), failure::Error> {
    ...
    // recv_timeout() -> タイムアウトするまでに受信するものがなければエラーになる
...
// リスト5.20
/**
 * ブロードキャストでDHCPクライアントにデータを送信する (簡略化のためブロードキャストを使用)
 */
pub fn send_dhcp_broadcast_response(
    soc: &UdpSocket, data: &[u8]) -> Result<(), failure::Error> {
...
pub fn obtain_static_addresses(
    ...
    let dhcp_server_address: Ipv4Addr = env
        .get("SERVER_IDENTIFIER")
        .expect("Missing server_identifier")
        .parse()?;

    let default_gateway: Ipv4Addr = env
        .get("DEFAULT_GATEWAY")
