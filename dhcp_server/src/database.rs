...
pub fn select_entry(
    ...
    "SELECT ip_addr FROM lease_entries WHERE mac_addr = ?1")?;

    let mut row = statement.query(params![mac_addr.to_string()])?;
...
pub fn select_addresses(
    ...
    if let Some(deleted) = deleted {
        let mut statement = con.prepare(
            "SELECT ip_addr FROM lease_entries WHERE deleted = ?"
        )?;
        ...
        let mut statement = con.prepare(
            "SELECT ip_addr FROM lease_entries"
        )?;
...
pub fn count_records_by_mac_addr(
    ...
    let mut statement = tx.prepare(
        "SELECT COUNT (*) FROM lease_entries WHERE mac_addr = ?"
    )?;
